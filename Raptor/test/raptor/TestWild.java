...
public class TestWild {
	...
	private void assertShortCastingMoves(Game game, PriorityMoveList<Move> moves) {
		Move m = null, m1 = null;
		for (Move move: moves) {
			if (move.isCastleShort()) {
				m = move;	
				game.move(m);
				PriorityMoveList<Move> moves1 = game.getPseudoLegalMoves();
	...
	private void assertLongCastingMoves(Game game, PriorityMoveList<Move> moves) {
		Move m = null, m1 = null;
		for (Move move: moves) {
