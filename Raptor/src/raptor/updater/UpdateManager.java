...
public class UpdateManager {
	...
	public static final String APP_HOME_DIR = ".raptor/";
        private static final String DOWNLOAD_URL = "http://raptor-chess-interface.googlecode.com/files/Raptor.jar";
	private static final String RAPTOR_FILENAME = "Raptor.jar";
	public static final File USER_RAPTOR_PREF = new File(
			System.getProperty("user.home") + "/" + APP_HOME_DIR
					+ "/raptor.properties");

        // This "isReadyToUpdate" is used instead of synchronization
        private Properties properties = new Properties();
	private boolean isReadyToUpdate = false;
	private static String appVersionStr = RaptorPreferenceStore.APP_VERSION_STRING;
	...
	//private static final String UPDATE_ACTIONS_URL = "http://dl.dropbox.com/u/46373738/updActions";
	private static final String UPDATE_ACTIONS_URL = null;
	...
	public static void invokeMain(String args[]) {
		File f = new File(RAPTOR_FILENAME);
		try {
	...
	private static void downloadFile(String urlPath, String toFilename) throws IOException {
		URL url = new URL(urlPath);
		ReadableByteChannel rbc2 = Channels.newChannel(url.openStream());
		try (FileOutputStream fos2 = new FileOutputStream(toFilename)) {
	...
	public void upgrade() {
		...
		try {
			...
			URL google = new URL(UPDATE_ACTIONS_URL);
			BufferedReader bin = new BufferedReader(new InputStreamReader(
					google.openStream()), 1024);
			String currentLine;
			while ((currentLine = bin.readLine()) != null) {
				if (currentLine.startsWith("for-version:")) {
                                        if (forVersionSet)
                                                break;
                                        if (currentLine.substring(13).equals(appVersionStr)) {
                                                forVersionSet = true;
                                        }
					...
					downloadFile(data[0], tempFilename);
					upgradeMade = true;
					if (isLikelyOSX)
			...
			if (!upgradeMade) {
				String tempFilename = System.getProperty("java.io.tmpdir")
						+ System.getProperty("file.separator")
						+ Math.abs(RAPTOR_FILENAME.hashCode());
				downloadFile(DOWNLOAD_URL, tempFilename);
				if (isLikelyOSX)
					applyOSX(tempFilename, RAPTOR_FILENAME);
				else if (isLikelyWindows)
					applyWindows(tempFilename, RAPTOR_FILENAME);
				else if (isLikelyLinux)
					applyLinux(tempFilename, RAPTOR_FILENAME);
			}
	...
	private void checkPrefs() {
                String path = USER_RAPTOR_PREF.getPath();
                try (InputStream stream = UpdateManager.class.getResourceAsStream(path)) {
                        properties.load(stream);
                        if (Boolean.valueOf(properties.getProperty("app-update", "true"))) {
                                appVersionStr = properties.getProperty("app-version");
                                isReadyToUpdate = Boolean.valueOf(properties.getProperty("ready-to-update"));
                        }
		} catch (IOException e) {
			e.printStackTrace();
		}
